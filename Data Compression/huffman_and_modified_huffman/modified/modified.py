...
def other(data,freq,numdata):
    others = 'others'
    otherfreq = 0
    prob = []
    for i in range(len(freq)) :
        prob.append(freq[i]/numdata)
    i = 0
    while i<len(prob):
        if(prob[i]<0.05):
            data.pop(i)
...
def compress():
    ...
    othersdata,othersfreq=other(chars,freq,len(Data))
    Dict = huffman_tree(othersdata,othersfreq)
    print(Dict)
    ...
    for i in Data:
        if (i in Dict.keys()):
            ...
            f2.write(Dict['others'])
            #f2.write(bin(ord(i)- 97))
            f2.write('{0:08b}'.format(ord(i)- 97))
